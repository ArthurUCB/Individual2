...
void salvar_playlist(const Playlist *playlist, const char *nome_arquivo) {
...
int comparar_musicas(const void *a, const void *b) {
    return strcmp(((const Musica *)a)->nome, ((const Musica *)b)->nome);
}
...
void adicionar_musica(Playlist *playlist, const char *nome, const char *artista, const char *album) {
    ...
    strncpy(playlist->musicas[playlist->total].nome, nome, TAM_NOME - 1);
    strncpy(playlist->musicas[playlist->total].artista, artista, TAM_NOME - 1);
    strncpy(playlist->musicas[playlist->total].album, album, TAM_NOME - 1);
    playlist->musicas[playlist->total].nome[TAM_NOME - 1] = '\0';
    playlist->musicas[playlist->total].artista[TAM_NOME - 1] = '\0';
    playlist->musicas[playlist->total].album[TAM_NOME - 1] = '\0';
    playlist->total++;
}
...
void buscar_musica(const Playlist *playlist, const char *nome) {
    for (int i = 0; i < playlist->total; i++) {
        if (strcmp(playlist->musicas[i].nome, nome) == 0) {
